    ...
    <div className="max-w-5xl mx-auto p-4 space-y-12 font-sans pb-20">
      ...
      <section className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="bg-slate-900 px-6 py-5 flex justify-between items-center">
          <h3 className="text-xl font-black text-white flex items-center gap-2">🍔 Burger Menu Admin</h3>
            ...
            <div key={nome} className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4">
                {itens.length > 0 ? itens.map((p) => {
                  ...
                  return (
                    <div key={p.id || realIdx} className={`flex flex-col md:flex-row gap-5 p-5 rounded-2xl border transition-all ${!p.available ? "bg-gray-50 border-gray-100 opacity-60" : "bg-white border-gray-100 hover:border-rose-200 hover:shadow-lg hover:shadow-rose-500/5"}`}>
                      <div className="relative shrink-0 mx-auto md:mx-0">
                        <img src={p.image || "https://via.placeholder.com/150"} alt={p.name}
                          className="w-28 h-28 md:w-32 md:h-32 rounded-2xl object-cover shadow-sm border border-gray-100" />
                      </div>
                      ...
                      <div className="flex md:flex-col justify-center items-center gap-2 pt-4 md:pt-0 border-t md:border-t-0 border-gray-100">
                        <BotoesAcao
                formRef={formRef}
                          disponivel={p.available}
                          onEditar={() => { setForm({ ...p, category: "Hamburguer" }); setEditingIdx(realIdx); }}
                          onToggle={() => { const next = { ...docData }; next.produtos[realIdx] = { ...p, available: !p.available }; saveDocData(next); }}
                          onExcluir={() => { if (confirm(`Excluir o burger ${p.name}?`)) { const next = { ...docData, produtos: docData.produtos.filter((_, i) => i !== realIdx) }; saveDocData(next); } }}
                        />
                      </div>
                    </div>
                  );
      ...
      <section className="max-w-2xl mx-auto">
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="p-6">
            <AdicionaisList items={docData.adicionais}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-64 overflow-y-auto pr-2">
        {items.map((item) => (
          <div key={item.id} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-100 group">
            ...
            <button onClick={() => onChange(items.filter((i) => i.id !== item.id))} className="text-slate-300 hover:text-rose-500 transition-colors p-1">✕</button>
          </div>
        ))}
      </div>
      <div className="flex flex-col gap-2 pt-4 border-t border-slate-100">
        <input placeholder="Nome (ex: Bacon Extra)" value={novo.nome} onChange={(e) => setNovo({ ...novo, nome: e.target.value })}
        ...
        <div className="flex gap-2">
          ...
          <button onClick={addItem} className="px-6 bg-slate-900 text-white rounded-xl font-black text-xs hover:bg-rose-600 transition-all uppercase tracking-widest">Add</button>
        </div>
      </div>
    </div>
  );
