    ...
    <div className="max-w-5xl mx-auto p-4 space-y-12 font-sans pb-20">
      ...
      <section className="bg-white rounded-3xl shadow-sm border border-stone-100 overflow-hidden">
        <div className="bg-emerald-900 px-6 py-5 border-b border-emerald-800 flex justify-between items-center">
          <h3 className="text-xl font-black text-emerald-50 flex items-center gap-2 uppercase tracking-tighter">
            🥙 Gestão de Esfihas
          </h3>
        </div>
                ...
                {itens.length > 0 ? (
                  itens.map((p) => {
                    ...
                    return (
                      <div
                        key={p.id || realIdx}
                        className={`flex flex-col sm:flex-row gap-5 p-4 rounded-2xl border transition-all duration-300 ${
                          !p.available 
                            ? "bg-stone-50 border-stone-100 opacity-60" 
                            : "bg-white border-stone-100 hover:border-emerald-200 hover:shadow-xl hover:shadow-emerald-900/5"
                        }`}
                      >
                        <img
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between items-start">
                            ...
                            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${p.available ? 'bg-emerald-100 text-emerald-700' : 'bg-stone-200 text-stone-500'}`}>
                        ...
                        <div className="flex sm:flex-col gap-2 justify-center border-t sm:border-t-0 pt-4 sm:pt-0 border-stone-50">
                          <button
                            onClick={() => { 
                            ...
                            className="flex-1 sm:w-10 sm:h-10 flex items-center justify-center rounded-xl bg-stone-100 text-stone-600 hover:bg-emerald-600 hover:text-white transition-all"
                          >
                            ✏️
                          </button>
                          <button
                            ...
                            className="flex-1 sm:w-10 sm:h-10 flex items-center justify-center rounded-xl bg-stone-100 text-stone-600 hover:bg-emerald-500 hover:text-white transition-all"
                          >
                            {p.available ? "👁️" : "🙈"}
                          </button>
                          <button
                            ...
                            className="flex-1 sm:w-10 sm:h-10 flex items-center justify-center rounded-xl bg-stone-100 text-stone-400 hover:bg-red-500 hover:text-white transition-all"
                          >
                            🗑️
                          </button>
                        </div>
...
function OptionList({ items = [], onChange }) {
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 gap-2 max-h-56 overflow-y-auto pr-2 scrollbar-thin">
        {items.map((item) => (
      ...
      <div className="pt-4 border-t border-stone-100 grid grid-cols-1 gap-2">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={addItem}
            className="px-6 bg-emerald-700 text-white rounded-xl font-black text-xs hover:bg-emerald-800 transition-all uppercase tracking-tighter shadow-lg shadow-emerald-900/10"
          >
            Adicionar
          </button>
        </div>
