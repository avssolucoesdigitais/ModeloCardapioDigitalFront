...
export default function PainelSalgados({ lojaId }) {
  ...
  const [novoProduto, setNovoProduto] = useState({
    nome: "", description: "", preco: "", image: "", available: true, sizes: [], prices: {},
  });
  ...
  function resetForm() {
    setNovoProduto({ nome: "", description: "", preco: "", image: "", available: true, sizes: [], prices: {} });
    setEditingIdx(null);
  }
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 pb-20 space-y-8 font-sans">
      ...
      {/* Formulário */}
      <section ref={formRef} className={`p-6 rounded-2xl shadow-xl transition-all duration-300 ${editingIdx !== null ? "bg-orange-50 border-2 border-orange-400" : "bg-white border border-gray-100"}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="space-y-4">
            <label className="block">
              ...
              <input placeholder="Ex: Coxinha de Frango" value={novoProduto.nome}
            ...
            <label className="block">
              ...
              <textarea placeholder="Ex: Massa de batata com recheio de frango desfiado..." value={novoProduto.description}
                onChange={(e) => setNovoProduto({ ...novoProduto, description: e.target.value })}
                className="mt-1 block w-full p-4 rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-orange-500 border outline-none transition-all shadow-sm" rows="3" />
            </label>
          </div>
          ...
          <div className="space-y-4">
            <label className="block">
              ...
              <input type="text" inputMode="decimal" placeholder="0,00" value={novoProduto.preco}
            ...
            <div className="flex items-center gap-4">
              <label className="flex-1 flex flex-col items-center justify-center h-32 border-2 border-dashed border-gray-300 rounded-xl hover:border-orange-500 cursor-pointer transition-colors bg-gray-50">
                {loadingImg ? <span className="animate-pulse text-orange-600 font-bold">Enviando...</span> : (
                  <><span className="text-3xl mb-1">📷</span><span className="text-xs font-bold text-gray-400">Foto do Salgado</span></>
                )}
              ...
              {novoProduto.image && (
                <div className="relative">
                  ...
                  <button onClick={() => setNovoProduto({ ...novoProduto, image: "" })} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-7 h-7 flex items-center justify-center shadow-lg">✕</button>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mt-8">
          <button onClick={addOrUpdateProduto} className="flex-1 h-14 bg-orange-600 hover:bg-orange-700 text-white font-black rounded-xl shadow-lg shadow-orange-100 transition-all active:scale-95">
          ...
          {docData.produtos.map((p, idx) => (
            <div key={p.id} className={`flex items-center gap-4 p-4 rounded-2xl bg-white border border-gray-100 shadow-sm hover:shadow-md transition-shadow ${!p.available && "opacity-50 grayscale"}`}>
              ...
              <BotoesAcao
                ...
                onEditar={() => { setNovoProduto({ ...p, preco: p.preco.toString() }); setEditingIdx(idx); }}
                onToggle={() => { const nd = { ...docData }; nd.produtos[idx].available = !nd.produtos[idx].available; saveDocData(nd); }}
                onExcluir={() => { if (confirm("Excluir salgado?")) saveDocData({ ...docData, produtos: docData.produtos.filter((i) => i.id !== p.id) }); }}
              />
            </div>
          ))}
        </div>
