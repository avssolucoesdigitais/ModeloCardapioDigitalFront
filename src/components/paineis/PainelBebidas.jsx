...
export default function PainelBebida({ lojaId }) {
  ...
  const [novoProduto, setNovoProduto] = useState({
    nome: "", description: "", image: "", preco: "", available: true, categoria: "tradicional", sizes: [], prices: {},
  });
  ...
  function resetForm() {
    setNovoProduto({ nome: "", description: "", image: "", preco: "", available: true, categoria: "tradicional", sizes: [], prices: {} });
    setEditingIdx(null);
  }
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 pb-20 space-y-8 font-sans text-gray-900">
      ...
      {/* Formulário */}
      <section ref={formRef} className={`p-6 rounded-2xl shadow-xl transition-all duration-300 ${editingIdx !== null ? "bg-indigo-50 border-2 border-indigo-400" : "bg-white border border-gray-100"}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="space-y-4">
            <label className="block">
              ...
              <input placeholder="Ex: Coca-Cola 350ml" value={novoProduto.nome}
            ...
            <label className="block">
              ...
              <input placeholder="Ex: Lata gelada ou Garrafa 2L" value={novoProduto.description}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <label>
                ...
                <select value={novoProduto.categoria}
            ...
            <div className="flex items-center gap-4">
              <label className="flex-1 flex flex-col items-center justify-center h-24 border-2 border-dashed border-gray-300 rounded-xl hover:border-indigo-500 cursor-pointer transition-colors bg-gray-50">
                {loadingImg ? <span className="animate-spin text-2xl">⏳</span> : (
                  <><span className="text-2xl">🖼️</span><span className="text-xs font-bold text-gray-400">Upload Imagem</span></>
                )}
        ...
        <div className="flex flex-col sm:flex-row gap-3 mt-8">
          <button onClick={addOrUpdateProduto} className="flex-1 h-14 bg-indigo-600 hover:bg-indigo-700 text-white font-black rounded-xl shadow-lg shadow-indigo-200 transition-all active:scale-95">
            ...
            <section key={cat}>
              ...
              <div className="grid grid-cols-1 gap-4">
                {itens.map((p) => {
                  ...
                  return (
                    <div key={p.id} className={`flex items-center gap-4 p-4 rounded-2xl bg-white border border-gray-100 shadow-sm transition-all hover:shadow-md ${!p.available && "opacity-50 grayscale"}`}>
                      <div className="relative">
                ...
                formRef={formRef}
                        ...
                        onExcluir={() => { if (confirm("Remover esta bebida?")) saveDocData({ ...docData, produtos: docData.produtos.filter((i) => i.id !== p.id) }); }}
                      />
                    </div>
                  );
                })}
