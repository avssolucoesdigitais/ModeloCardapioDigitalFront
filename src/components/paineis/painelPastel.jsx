    ...
    <div className="max-w-5xl mx-auto p-4 space-y-12 font-sans pb-20">
      ...
      <section className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="bg-gray-50 px-6 py-4 border-b border-gray-100">
          <h3 className="text-xl font-black text-gray-800">🥟 Cardápio de Pastéis</h3>
        </div>
...
function SimpleItemSection({ title, items = [], onChange }) {
  ...
  function addItem() {
    ...
    const preco = parsePreco(novo.preco);
    onChange([...items, { ...novo, preco, id: Date.now() }]);
    setNovo({ nome: "", preco: "" });
  }
    ...
    <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <div className="p-6 space-y-4">
        <div className="grid grid-cols-1 gap-2 max-h-60 overflow-y-auto pr-2">
          {items.map((item) => (
            <div key={item.id} className="flex items-center justify-between p-3 rounded-xl bg-gray-50 border border-transparent hover:border-orange-100 hover:bg-white transition-all">
