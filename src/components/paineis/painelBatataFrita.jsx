...
export default function PainelBatata({ lojaId }) {
  ...
  const [novoProduto, setNovoProduto] = useState({
    nome: "", description: "", image: "", sizes: [], prices: {}, preco: "", available: true, categoria: "tradicional",
  });
  ...
  function resetForm() {
    setNovoProduto({ nome: "", description: "", image: "", sizes: [], prices: {}, preco: "", available: true, categoria: "tradicional" });
    setEditingIdx(null);
  }
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 pb-20 space-y-8">
      ...
      {/* Formulário */}
      <section ref={formRef} className={`p-6 rounded-2xl shadow-lg transition-all ${editingIdx !== null ? "bg-amber-50 border-2 border-amber-400" : "bg-white border border-gray-100"}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-4">
            <label className="block">
              ...
              <input placeholder="Ex: Batata Cheddar e Bacon" value={novoProduto.nome}
            ...
            <label className="block">
              ...
              <textarea placeholder="Descreva os ingredientes..." value={novoProduto.description}
                onChange={(e) => setNovoProduto({ ...novoProduto, description: e.target.value })}
                className="mt-1 block w-full p-4 rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-amber-500 border outline-none transition-all" rows="3" />
            </label>
          </div>
            ...
            <div className="flex items-center gap-4 pt-2">
              <label className="flex-1 flex flex-col items-center justify-center h-24 border-2 border-dashed border-gray-300 rounded-xl hover:border-amber-500 cursor-pointer transition-colors bg-gray-50">
                {loadingImg ? <span className="animate-pulse text-amber-600 font-medium">Subindo...</span> : (
                  <><span className="text-2xl">📸</span><span className="text-xs text-gray-500">Foto do prato</span></>
                )}
        ...
        <div className="flex gap-3 mt-8">
          <button onClick={addOrUpdateProduto} className="flex-1 h-12 bg-amber-500 hover:bg-amber-600 text-white font-bold rounded-xl shadow-md transition-transform active:scale-95">
            ...
            <section key={cat}>
              ...
              <div className="grid grid-cols-1 gap-3">
                ...
                formRef={formRef}
                        ...
                        onExcluir={() => { if (confirm("Excluir item?")) saveDocData({ ...docData, produtos: docData.produtos.filter((i) => i.id !== p.id) }); }}
                      />
                    </div>
                  );
                })}
