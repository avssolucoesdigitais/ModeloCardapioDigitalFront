...
export default function PainelPromocao() {
  const [docData, setDocData] = useState({ produtos: [] }); // salva em 'produtos'
  ...
  return (
    <div className="space-y-10">
      ...
      <section className="border p-4 rounded-md bg-gray-50 space-y-3">
        <h2 className="text-xl font-bold mb-2">🔥 Promoção</h2>
        <input
          placeholder="Nome"
        ...
        {novoProduto.image && (
          <img src={novoProduto.image} alt="preview" className="w-24 h-24 object-cover rounded" />
        )}
        <button
      ...
      <section className="space-y-3">
        ...
        {Array.isArray(docData.produtos) && docData.produtos.length > 0 ? (
          docData.produtos.map((p, idx) => (
            <div
              key={p.id}
              className="flex flex-col sm:flex-row gap-4 items-start border rounded-lg p-3 bg-white shadow-sm"
            >
