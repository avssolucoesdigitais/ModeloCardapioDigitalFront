...
export default function PainelSalgadinhos({ lojaId }) {
  ...
  useEffect(() => {
    (async () => {
      const ref = doc(db, "lojas", lojaId, "opcoes", "Salgadinhos");
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 pb-24 space-y-8 font-sans">
      ...
      {/* Formulário */}
      <section ref={formRef} className={`p-6 rounded-2xl shadow-xl transition-all duration-300 ${editingIdx !== null ? "bg-yellow-50 border-2 border-yellow-400" : "bg-white border border-gray-100"}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            <label className="block">
              ...
              <input placeholder="Nome do produto" value={novoProduto.nome}
            ...
            <label className="block">
              ...
              <input placeholder="Ex: Porção com 100 unidades" value={novoProduto.desc}
          ...
          <div className="space-y-4">
            <label className="block">
              ...
              <input type="text" inputMode="decimal" placeholder="0,00" value={novoProduto.preco}
                ...
                className="mt-1 block w-full h-12 px-4 rounded-xl border-gray-200 bg-gray-50 border outline-none focus:ring-2 focus:ring-yellow-500 shadow-sm" />
            </label>
            <div className="flex items-center gap-4 pt-1">
              <label className="flex-1 flex flex-col items-center justify-center h-24 border-2 border-dashed border-gray-300 rounded-xl hover:border-yellow-500 cursor-pointer transition-colors bg-gray-50">
                {loadingImg ? <span className="text-yellow-600 animate-bounce">⬆️</span> : (
                  <><span className="text-2xl">📸</span><span className="text-[10px] uppercase font-bold text-gray-400">Imagem</span></>
                )}
        ...
        <div className="flex flex-col sm:flex-row gap-3 mt-8">
          <button onClick={addOrUpdateProduto} className="flex-1 h-14 bg-yellow-500 hover:bg-yellow-600 text-white font-black rounded-xl shadow-lg shadow-yellow-100 transition-all active:scale-95">
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {docData.produtos.map((p, idx) => (
            <div key={p.id} className={`flex items-center gap-4 p-4 rounded-2xl bg-white border border-gray-100 shadow-sm hover:shadow-md transition-all ${!p.available && "opacity-50 grayscale"}`}>
              <img src={p.image || "https://via.placeholder.com/80"} className="w-16 h-16 rounded-lg object-cover shrink-0" alt={p.nome} />
              ...
              <BotoesAcao
                ...
                onExcluir={() => { if (confirm("Excluir porção?")) saveDocData({ ...docData, produtos: docData.produtos.filter((i) => i.id !== p.id) }); }}
              />
            </div>
          ))}
        </div>
