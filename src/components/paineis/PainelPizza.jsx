...
export default function PainelPizza({ lojaId }) {
  ...
  function resetForm() {
    setForm({ name: "", description: "", image: "", sizes: [], prices: {}, available: true, categoria: "" });
    setEditingIdx(null);
  }
    ...
    <div className="max-w-5xl mx-auto p-4 space-y-12 font-sans pb-20">
      ...
      <section className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="bg-gray-50 px-6 py-4 border-b border-gray-100">
          <h3 className="text-xl font-black text-gray-800">🍕 Pizzas no Cardápio</h3>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <SimpleItemSection title="🥖 Bordas Recheadas" items={docData.bordas} color="amber"
          onChange={(items) => { const next = { ...docData, bordas: items }; setDocData(next); saveDocData(next); }} />
        <SimpleItemSection title="🧀 Adicionais Extras" items={docData.adicionais} color="blue"
          onChange={(items) => { const next = { ...docData, adicionais: items }; setDocData(next); saveDocData(next); }} />
      </div>
    </div>
  );
}
...
function SimpleItemSection({ title, items = [], onChange, color }) {
  ...
  function addItem() {
    ...
    const preco = parseFloat(String(novo.preco).replace(",", ".").replace(/[^\d.]/g, "")) || 0;
    onChange([...items, { nome: novo.nome, preco, id: Date.now() }]);
    setNovo({ nome: "", preco: "" });
  }
  ...
  return (
    <section className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-full">
      ...
      <div className="p-6 flex-1 flex flex-col">
        <div className="space-y-2 mb-6 flex-1 overflow-y-auto max-h-[300px] pr-2">
          {items.map((item) => (
            <div key={item.id} className="group flex items-center justify-between border border-gray-50 rounded-xl p-3 bg-gray-50 hover:bg-white hover:border-gray-200 transition-all">
              <div className="flex flex-col">
                ...
                <span className={`text-xs font-black ${color === "amber" ? "text-amber-600" : "text-blue-600"}`}>+ R$ {Number(item.preco).toFixed(2).replace(".", ",")}</span>
              </div>
              <button onClick={() => onChange(items.filter((i) => i.id !== item.id))} className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-300 hover:text-red-500 hover:bg-red-50 transition-all">✕</button>
            </div>
          ))}
