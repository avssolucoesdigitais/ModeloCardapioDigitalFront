      ...
      <div className="w-[420px] h-full overflow-y-auto shadow-2xl flex flex-col p-6 bg-white">
        ...
        <div className="flex-1 space-y-4">
          ...
          {cart.items.map((l, idx) => (
            ...
            >
              {l.image && (
                <img src={l.image} alt={l.name} className="w-16 h-16 object-cover rounded-xl" />
              )}
              ...
              <div className="flex-1">
                ...
                <div className="text-sm text-gray-500">{formatBRL(l.price)}</div>

                <div className="mt-2 flex items-center gap-2">
                  <button className="p-1 rounded-lg border hover:bg-gray-50" onClick={() => cart.dec(l.id, l.size)}>
                    <Minus className="w-4 h-4" />
                  </button>
                  <span className="w-8 text-center">{l.qty}</span>
                  <button className="p-1 rounded-lg border hover:bg-gray-50" onClick={() => cart.add(l)}>
              ...
              <div className="text-right">
                <div className="font-medium text-gray-900">
                  {formatBRL(parsePreco(l.price) * l.qty)}
                </div>
                <button
