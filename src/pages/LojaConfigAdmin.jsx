...
// Chaves legadas (sem acento) que podem existir no Firestore de lojas antigas
...
const HORARIOS_VAZIOS = Object.fromEntries(
  DIAS_ORDENADOS.map(d => [d.key, { abre: "", fecha: "" }])
);

// Normaliza horários vindos do Firestore:
// migra chaves legadas e garante que todas as chaves acentuadas existam
function normalizeHorarios(raw = {}) {
  const result = { ...HORARIOS_VAZIOS };
  Object.entries(raw).forEach(([k, v]) => {
    const key = LEGACY_KEY_MAP[k] ?? k;
    if (key in result) result[key] = { abre: v?.abre || "", fecha: v?.fecha || "" };
  });
  return result;
}

// Nunca deixa lojaId undefined chegar ao Firestore
function assertLojaId(lojaId) {
  if (!lojaId) throw new Error("lojaId é obrigatório em LojaConfigAdmin");
}

export default function LojaConfigAdmin({ lojaId }) {
    ...
    (async () => {
      ...
      if (snap.exists()) {
        ...
        setConfig(prev => ({
          ...prev,
          ...data,
          // merge explícito: normaliza chaves legadas e garante todos os dias
  ...
  if (!lojaId) return <div className="p-20 text-center font-bold text-red-500">Erro: lojaId não informado.</div>;
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
          <section className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-4">
                ...
                <div className="relative h-40 bg-slate-100 rounded-3xl overflow-hidden border-2 border-dashed border-slate-200">
                  {config.bannerUrl
          ...
          <section className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100">
            ...
            <div className="flex gap-2 mb-6">
              <input type="text" placeholder="CÓDIGO" className="bg-slate-50 border-none rounded-xl p-3 text-sm font-bold flex-1"
                value={cupom.codigo} onChange={e => setCupom({...cupom, codigo: e.target.value.toUpperCase()})} />
              <input type="number" placeholder="R$ 0,00" className="bg-slate-50 border-none rounded-xl p-3 text-sm font-bold w-32"
                value={cupom.valor} onChange={e => setCupom({...cupom, valor: e.target.value})} />
              <button onClick={() => {
            ...
            <div className="flex flex-wrap gap-2">
              {config.cupons?.map(c => (
                <div key={c.codigo} className="bg-amber-50 border border-amber-100 px-4 py-2 rounded-2xl flex items-center gap-3">
                  ...
                  <button onClick={() => updateFirestore({...config, cupons: config.cupons.filter(x => x.codigo !== c.codigo)})}
                    className="text-amber-300 hover:text-amber-600">✕</button>
                </div>
              ))}
            </div>
          </section>

          <button onClick={() => updateFirestore(config)} disabled={saving}
            className="fixed bottom-6 right-6 md:relative md:bottom-0 md:right-0 w-full md:w-auto bg-blue-600 text-white px-10 py-4 rounded-2xl font-black shadow-xl shadow-blue-200 transition-all hover:scale-105 active:scale-95">
      ...
      {activeTab === "bairros" && (
        <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100 animate-in fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            <input type="text" placeholder="Nome do Bairro" className="bg-slate-50 border-none rounded-2xl p-4 text-sm font-bold"
              value={bairro.nome} onChange={e => setBairro({...bairro, nome: e.target.value})} />
            <input type="number" placeholder="Taxa R$" className="bg-slate-50 border-none rounded-2xl p-4 text-sm font-bold"
              value={bairro.taxa} onChange={e => setBairro({...bairro, taxa: e.target.value})} />
            <button onClick={() => {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {config.bairros?.map(b => (
              <div key={b.nome} className="flex justify-between items-center bg-slate-50 p-4 rounded-2xl group border border-transparent hover:border-slate-200 transition-all">
                ...
                <button onClick={() => updateFirestore({...config, bairros: config.bairros.filter(x => x.nome !== b.nome)})}
                  className="p-2 bg-white rounded-xl shadow-sm opacity-0 group-hover:opacity-100 text-rose-500 transition-all">✕</button>
              </div>
            ))}
          </div>
        </div>
...
const CustomInput = ({ label, ...props }) => (
  <div className="space-y-1">
    ...
    <input {...props} onChange={e => props.onChange(e.target.value)}
      className="w-full bg-slate-50 border-none rounded-2xl p-3 text-sm font-bold focus:ring-2 focus:ring-blue-100 transition-all" />
  </div>
);
